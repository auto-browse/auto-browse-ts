...
export class Context {
  ...
  constructor(tools: Tool[] = [], config: FullConfig = defaultConfig, browserContextFactory?: BrowserContextFactory) {
    this.tools = tools;
    this.config = config;
  ...
  async allFramesSnapshot() {
    ...
    const snapshots = await Promise.all([
      ...
      ...lastSnapshotFrames.map(async (frame, index) => {
        const snapshot = await (frame.locator('html') as any).ariaSnapshot({
          ref: true
        });
        const args = [];
